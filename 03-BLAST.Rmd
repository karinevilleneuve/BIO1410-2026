...
avec les librairies qui vous seront utiles ains 

1. Créer un nouveau document de type R markdown. 

2. Insérer un premier bloc de code R pour charger les librairies nécessaires 

Bloc de code 1 : Installation des packages/libraires 

3. Créer un second bloc de code R et y insérer chacune des commandes suivantes requises pour effectuer chacune des opérations 

a. Récupérer le fichier csv à partir du lien suivant : 
...
À la première ligne du code, copier/coller le nom de votre scientifique à partir de la liste des scienfitiques disponible après les blocs des code. Attention, assurez-vous **de ne pas** avoir d'espace entre les guillemets et le nom. 
...
n = nrow(data.frame(unique(df$Genus))) # On calcule le nombre de genre unique que nous avons 
